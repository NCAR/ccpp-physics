...
!!
      subroutine cu_gf_driver_run(tottracer,ntrac,garea,im,ix,km,dt, &
...
!-------------------------------------------------------------
   implicit none
      ...
      integer, parameter :: ichoice=13	! 0 2 5 13 8
...
!hj define locally for now.
   integer, dimension(im) :: cactiv ! hli for gf
!hj change from ix to im
...
!
!     print*,'hli in gf cactiv',cactiv(1)
     cactiv(:)      = 0
     rand_mom(:)    = 0.
     rand_vmas(:)   = 0.
     rand_clos(:,:) = 0.
