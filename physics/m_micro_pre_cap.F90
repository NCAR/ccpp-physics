...
!>
!! @brief Auto-generated cap module for the m_micro_pre scheme
!!
!
module m_micro_pre_cap
    ...
    use            :: m_micro_pre, &
                      only: m_micro_pre_init,m_micro_pre_finalize,m_micro_pre_run
    ...
    private
    public :: m_micro_pre_init_cap,m_micro_pre_finalize_cap,m_micro_pre_run_cap

    contains


    function m_micro_pre_init_cap(ptr) bind(c) result(ierr)
        ...
        call m_micro_pre_init()
        

    end function m_micro_pre_init_cap

    function m_micro_pre_finalize_cap(ptr) bind(c) result(ierr)
        ...
        call m_micro_pre_finalize()
        

    end function m_micro_pre_finalize_cap

    function m_micro_pre_run_cap(ptr) bind(c) result(ierr)
        ...
        integer, pointer :: imfdeepcnv
        integer, pointer :: imfshalcnv
        real(kind_phys), pointer :: gq0_ice(:,:)
        real(kind_phys), pointer :: gq0_water(:,:)
        ...
        real(kind_phys), pointer :: cld_shoc(:,:)
        ...
        call ccpp_field_get(cdata, 'flag_for_mass_flux_deep_convection_scheme', imfdeepcnv, ierr=ierr, kind=ckind, index=290)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve flag_for_mass_flux_deep_convection_scheme from CCPP data structure')
            return
        end if
        if (kind(imfdeepcnv).ne.ckind) then
            call ccpp_error('Kind mismatch for variable flag_for_mass_flux_deep_convection_scheme')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'ice_water_mixing_ratio_updated_by_physics', gq0_ice, ierr=ierr, dims=cdims, kind=ckind, index=376)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve ice_water_mixing_ratio_updated_by_physics from CCPP data structure')
            return
        end if
        if (kind(gq0_ice).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'cloud_condensed_water_mixing_ratio_updated_by_physics', gq0_water, ierr=ierr, dims=cdims, kind=ckind, index=95)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve cloud_condensed_water_mixing_ratio_updated_by_physics from CCPP data structure')
            return
        end if
        if (kind(gq0_water).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'subgrid_scale_cloud_fraction_from_shoc', cld_shoc, ierr=ierr, dims=cdims, kind=ckind, index=675)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve subgrid_scale_cloud_fraction_from_shoc from CCPP data structure')
            return
        end if
        if (kind(cld_shoc).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'cloud_fraction_for_MG', cld_frc_MG, ierr=ierr, dims=cdims, kind=ckind, index=99)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve cloud_fraction_for_MG from CCPP data structure')
            return
        end if
        if (kind(cld_frc_MG).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'cloud_condensed_water_mixing_ratio_convective_transport_tracer', clw_water, ierr=ierr, dims=cdims, kind=ckind, index=93)
#ifdef DEBUG
        if (ierr /= 0) then
        ...
        if (kind(clw_water).ne.ckind) then
        ...
        call m_micro_pre_run(im=im,levs=levs,do_shoc=do_shoc,fprcp=fprcp,mg3_as_mg2=mg3_as_mg2,imfdeepcnv=imfdeepcnv, &
                  imfshalcnv=imfshalcnv,gq0_ice=gq0_ice,gq0_water=gq0_water,gq0_rain=gq0_rain, &
                  gq0_snow=gq0_snow,gq0_graupel=gq0_graupel,gq0_rain_nc=gq0_rain_nc,gq0_snow_nc=gq0_snow_nc, &
                  gq0_graupel_nc=gq0_graupel_nc,cld_shoc=cld_shoc,cnvc=cnvc,cnvw=cnvw,tcr=tcr, &
                  tcrf=tcrf,gt0=gt0,qrn=qrn,qsnw=qsnw,qgl=qgl,ncpr=ncpr,ncps=ncps,ncgl=ncgl, &
                  cld_frc_MG=cld_frc_MG,qlcn=qlcn,qicn=qicn,cf_upi=cf_upi,clw_water=clw_water, &
                  clw_ice=clw_ice,clcn=clcn,errmsg=cdata%errmsg,errflg=cdata%errflg)
        ierr=cdata%errflg

    end function m_micro_pre_run_cap
end module m_micro_pre_cap
