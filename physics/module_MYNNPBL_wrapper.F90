...
!!
!> \brief This scheme (1) performs pre-mynnedmf work, (20 runs the mynnedmf, and (3) performs post-mynnedmf work
#if 0
...
!! | QC_BL               | subgrid_cloud_mixing ratio_pbl                                              | subgrid cloud cloud mixing ratio from PBL scheme      | kg kg-1       |    2 | real      | kind_phys | inout  | F        |
...
!! | grav_settling       | grav_settling                                                               | switch to activate gravitational setting of fog       | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_tkebudget   | tke_budget                                                                  | switch for activating TKE budget                      | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_tkeadvect   | tke_advect                                                                  | switch for activating TKE advect                      | switch        |    0 | logical   |           | in     | F        |
!! | bl_mynn_cloudpdf    | cloudpdf                                                                    | switch to determine which cloud PDF to use            | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_mixlength   | mixing_length_switch                                                        | switch to determine which mixing length form to use   | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_edmf        | edmf_switch                                                                 | switch to activate the mass-flux scheme               | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_edmf_mom    | edmf_momentum_transport_switch                                              | switch to activate the transport of momentum          | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_edmf_tke    | edmf_tke_transport_switch                                                   | switch to activate the transport of TKE               | switch        |    0 | integer   |           | in     | F        |
!! | bl_mynn_edmf_part   | edmf_partition_switch                                                       | switch to partitioning of the MF and ED areas         | switch        |    0 | integer   |           | in     | F        |
