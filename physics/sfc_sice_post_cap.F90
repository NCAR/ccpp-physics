...
!>
!! @brief Auto-generated cap module for the sfc_sice_post scheme
!!
!
module sfc_sice_post_cap
    ...
    use            :: sfc_sice_post, &
                      only: sfc_sice_post_finalize,sfc_sice_post_run,sfc_sice_post_init
    ...
    private
    public :: sfc_sice_post_finalize_cap,sfc_sice_post_run_cap,sfc_sice_post_init_cap

    contains


    function sfc_sice_post_finalize_cap(ptr) bind(c) result(ierr)
        ...
        call sfc_sice_post_finalize()
        

    end function sfc_sice_post_finalize_cap

    function sfc_sice_post_run_cap(ptr) bind(c) result(ierr)
        ...
        call ccpp_field_get(cdata, 'sea_ice_temperature', tisfc, ierr=ierr, dims=cdims, kind=ckind, index=629)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve sea_ice_temperature from CCPP data structure')
            return
        end if
        if (kind(tisfc).ne.ckind) then
        ...
        call sfc_sice_post_run(im=im,islmsk=islmsk,tsfc=tsfc,fice=fice,hice=hice,tisfc=tisfc,errmsg=cdata%errmsg, &
                  errflg=cdata%errflg)
        ierr=cdata%errflg

    end function sfc_sice_post_run_cap

    function sfc_sice_post_init_cap(ptr) bind(c) result(ierr)
        ...
        call sfc_sice_post_init()
        

    end function sfc_sice_post_init_cap
end module sfc_sice_post_cap
