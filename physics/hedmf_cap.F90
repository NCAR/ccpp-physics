...
!>
!! @brief Auto-generated cap module for the hedmf scheme
!!
!
module hedmf_cap
    ...
    use            :: hedmf, &
                      only: hedmf_run,hedmf_finalize,hedmf_init
    ...
    private
    public :: hedmf_run_cap,hedmf_finalize_cap,hedmf_init_cap

    contains


    function hedmf_run_cap(ptr) bind(c) result(ierr)
        ...
        real(kind_phys), pointer :: hgamt(:)
        real(kind_phys), pointer :: hgamq(:)
        ...
        integer, pointer :: ipr
        real(kind_phys), pointer :: xkzminv
        real(kind_phys), pointer :: moninq_fac
        ...
        call ccpp_field_get(cdata, 'flag_TKE_dissipation_heating', dspheat, ierr=ierr, kind=ckind, index=258)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve flag_TKE_dissipation_heating from CCPP data structure')
            return
        end if
        if (kind(dspheat).ne.ckind) then
            call ccpp_error('Kind mismatch for variable flag_TKE_dissipation_heating')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'countergradient_mixing_term_for_temperature', hgamt, ierr=ierr, dims=cdims, kind=ckind, index=137)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve countergradient_mixing_term_for_temperature from CCPP data structure')
            return
        end if
        if (kind(hgamt).ne.ckind) then
            call ccpp_error('Kind mismatch for variable countergradient_mixing_term_for_temperature')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'countergradient_mixing_term_for_water_vapor', hgamq, ierr=ierr, dims=cdims, kind=ckind, index=138)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve countergradient_mixing_term_for_water_vapor from CCPP data structure')
            return
        end if
        if (kind(hgamq).ne.ckind) then
            call ccpp_error('Kind mismatch for variable countergradient_mixing_term_for_water_vapor')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'atmosphere_heat_diffusivity_background_maximum', xkzminv, ierr=ierr, kind=ckind, index=70)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve atmosphere_heat_diffusivity_background_maximum from CCPP data structure')
            return
        end if
        if (kind(xkzminv).ne.ckind) then
            call ccpp_error('Kind mismatch for variable atmosphere_heat_diffusivity_background_maximum')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'atmosphere_diffusivity_coefficient_factor', moninq_fac, ierr=ierr, kind=ckind, index=67)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve atmosphere_diffusivity_coefficient_factor from CCPP data structure')
            return
        end if
        if (kind(moninq_fac).ne.ckind) then
            call ccpp_error('Kind mismatch for variable atmosphere_diffusivity_coefficient_factor')
            ierr = 1
            return
        end if
#endif
        ...
        call hedmf_run(ix=ix,im=im,km=km,ntrac=ntrac,ntcw=ntcw,dv=dv,du=du,tau=tau,rtg=rtg,u1=u1, &
                  v1=v1,t1=t1,q1=q1,swh=swh,hlw=hlw,xmu=xmu,psk=psk,rbsoil=rbsoil,zorl=zorl, &
                  u10m=u10m,v10m=v10m,fm=fm,fh=fh,tsea=tsea,heat=heat,evap=evap,stress=stress, &
                  spd1=spd1,kpbl=kpbl,prsi=prsi,del=del,prsl=prsl,prslk=prslk,phii=phii,phil=phil, &
                  delt=delt,dspheat=dspheat,dusfc=dusfc,dvsfc=dvsfc,dtsfc=dtsfc,dqsfc=dqsfc, &
                  hpbl=hpbl,hgamt=hgamt,hgamq=hgamq,dkt=dkt,kinver=kinver,xkzm_m=xkzm_m,xkzm_h=xkzm_h, &
                  xkzm_s=xkzm_s,lprnt=lprnt,ipr=ipr,xkzminv=xkzminv,moninq_fac=moninq_fac, &
                  errmsg=cdata%errmsg,errflg=cdata%errflg)
        ierr=cdata%errflg

    end function hedmf_run_cap

    function hedmf_finalize_cap(ptr) bind(c) result(ierr)
    ...
    end function hedmf_finalize_cap

    function hedmf_init_cap(ptr) bind(c) result(ierr)
    ...
    end function hedmf_init_cap
end module hedmf_cap
