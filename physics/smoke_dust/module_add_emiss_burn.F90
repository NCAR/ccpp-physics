...
CONTAINS
  subroutine add_emis_burn(dtstep,dz8w,rho_phy,pi,                  &
                           ...
                           coef_bb_dc, fhist, hwp, hwp_prevd,       & 
   ...
   real(kind_phys), DIMENSION(ims:ime,jms:jme), INTENT(IN) :: hwp, peak_hr, fire_end_hr, ebu_in !, vfrac
   ...
   real(kind_phys), DIMENSION(ims:ime,jms:jme), INTENT(INOUT) :: fhist
!>--local 
   integer :: i,j,k,n,m
...
! applied! 
    ...
    ! Constants for the fire diurnal cycle calculation
     do j=jts,jte
       do i=its,ite
        fire_age= time_int + (fire_end_hr(i,j))*3600.
