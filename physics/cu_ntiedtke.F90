...
!! Zhang et al. (2011), Zhang and Wang (2017, 2018)
!!
!###########################################################
...
!!
      subroutine cu_ntiedtke_init(mpirank, mpiroot, errmsg, errflg)
         ...
         if (mpirank==mpiroot) then
            write(0,*) ' -----------------------------------------------------------------------------------------------------------------------------'
            write(0,*) ' --- WARNING --- the CCPP New Tiedtke convection scheme is currently under development, use at your own risk --- WARNING ---'
            write(0,*) ' -----------------------------------------------------------------------------------------------------------------------------'
         end if
         ! *DH temporary
...
!
      do k=1,km
        ...
        do j=1,lq
          ...
          pverv(j,k1)=pomg(j,k)
          pgeo(j,k1)=poz(j,k)
          pgeoh(j,k1)=pzz(j,k+1)
          pap(j,k1)=prsl(j,k)
          paph(j,k1)=prsi(j,k+1)
          tt=ztp1(j,k1)
          zew  = foeewm(tt)                                      
          zqs  = zew/pap(j,k1)                                      
          zqs  = min(0.5,zqs)                                         
...
!      print *, "pqte=",pqte(1,:)
!      print *, "ptte=",ptte(1,:)
!      print *, "hfx=", pqhfl(1),phhfl(1),dx(1)
!
!-----------------------------------------------------------------------
...
!
      do k=1,km
        ...
        do j=1,lq
          ...
          cnvc(j,k) = min(cnvc(j,k), 0.6)
          cnvc(j,k) = max(cnvc(j,k), 0.0)
        end do
      end do
...
! ---------------------------------------------------
 ...
 ! -------------------------
    ...
    if (  ktrac > 0 ) then
      ...
      call cuctracer(klon,klev,ktrac,kctop,idtop,   &
                     lldcum,llddraf3,ztmst,paph,zmfuus,zmfdus, &
...
! the next levels, we use the variables at the first level as initial values
      do jl=1,klon
      if(loflag(jl)) then
        eta(jl) = 0.55/(pgeo(jl,jk)*zrg)+1.e-4
...
! assume the mix-layer is 60hPa
      ...
      do levels=klevm1-1,klevm1-20,-1 ! loop starts
        do jk=1,klev
          do jl=1,klon
...
! mixing and "pressure" gradient term in upper troposphere
              ...
              if ( zbuo(jl,jk) < 0. ) then
                ...
                zmfun = sqrt(zkedke)*pmfu(jl,jk+1) !* (1.6-min(1.,pqen(jl,jk) /    &
                      !  pqsen(jl,jk)))
...
!  level 3 souroutines
!--------------------------------------------------------
    subroutine cuctracer(klon,klev,ktrac,kctop,kdtop,           &
                       ldcum,lddraf,ztmst,paph,pmfu,pmfd,       &
                       pudrate,pddrate,pcen,ptenc)
    implicit none
    integer  klon,klev,ktrac
    integer  kctop(klon), kdtop(klon)
    logical  ldcum(klon), lddraf(klon)
    real(kind=kind_phys)   ztmst
    real(kind=kind_phys)   paph(klon,klev+1)
    real(kind=kind_phys)   pmfu(klon,klev)
    real(kind=kind_phys)   pmfd(klon,klev)
    real(kind=kind_phys)   pudrate(klon,klev)
    real(kind=kind_phys)   pddrate(klon,klev)
    real(kind=kind_phys)   pcen(klon,klev,ktrac)
    real(kind=kind_phys)   ptenc(klon,klev,ktrac)
    !----------------------------------------------------------------------
    integer ik , jk , jl , jn
    real(kind=kind_phys) zzp , zmfa , zerate , zposi
    ! ALLOCATABLE ARAYS
    real(kind=kind_phys) , dimension(:,:,:) , allocatable :: zcen , zcu , zcd , &
                               ztenc , zmfc
    real(kind=kind_phys) , dimension(:,:) , allocatable :: zdp
...
!----------------------------------------------------------------------             
      ...
      integer  klev,klon                                                                   
      real(kind=kind_phys)     pt(klon,klev),          pq(klon,klev),  &                             
     &         psp(klon)                                                            
      logical  ldflag(klon)                                                         
! local variables
      integer  jl,jk
...
!
     ...
     &        (1.-foealfa(tt))*ralsdcp
      ...
      real(kind=kind_phys) function foeldcp(tt)
        implicit none
        real(kind=kind_phys) tt
        foeldcp = foedelta(tt)*ralvdcp + (1.-foedelta(tt))*ralsdcp
      end function foeldcp

      real(kind=kind_phys) function foedelta(tt)
        implicit none
        real(kind=kind_phys) tt
