...
!             con_g, con_omega, con_rd, con_cp, con_rv,con_pi, con_rerth, con_fvirt,   &
!                      xlat_d, sinlat, coslat, area,  &
!                      cdmbgwd(1:2), hprime, oc, oa4, clx, theta,          &
!                      sigma, gamma, elvmax,  varss,  kpbl,          &		     
...
!      use cires_ugwpv1_sporo, only       : oro_spectral_solver
...
!----------------------------------------      
      ...
      real(kind=kind_phys), intent(in) :: grav, con_omega, rd, cpd, rv, pi, arad, fv 
...
!===========================
! scalars from phys-contants added by "CCPP-team"
!   by rejecting to use "ugwp_common"
!===========================     
      rcpdt = 1.0 / (cpd*dtp)
      grav2 = grav + grav
!
...
!	  
	  ...
	  gamma(i) = aelps/belps
        ...
        if (do_adjoro ) then   
!	
! more adjustments "lengths", gamma and sigma, valid assuminng H=2*hprime H/2 = hprime
!	      
...
!=======================================================================          
      ...
      do i = 1, npt
        j = ipt(i)
!	
!gfsv15/16:   ELVMAX(J) = min (ELVMAX(J) + sigfac * hprime(j), hncrit=8000.)	Max-level of SSO-HILL
!
...
!
! place here computation for "ktur" and ogw-dissipation for the spectral ORO-scheme
!	    
        enddo
      enddo
      k = 1
...
!         zobl = max(heff*(1. -fcrit_v1/fr_Flow), 0)
! 
!
          ...
          fr      = heff*bnv/ulow(i)
          zw1     = max(heff*(1. -fcrit_v1/fr), 0.0)
          zw2     = zmet(j,2)
	
         if (fr > fcrit_v1 .and. zw1 > zw2 ) then 
           do k=2, kmm1
            zmetp =  zmet(j,k+1) 
            zmetk   =  zmet(j,k)   
...
!     
      ...
      enddo
!
! iwk - adhoc criteria to select ghe ogw-launch level between
...
!                      the top hill can be inside PBL.... if kref = khtop
!
...
!     
      do i = 1,npt
       ...
       if (heff <= 0) cycle
        zw1 = ulow(i)/bnv
        hsat = fcrit_v1 *zw1              
        heff = min(heff, hsat)                             ! similar hsat-limit in CAM as found in Dec 2020
...
!	if (kdt == 1) then
!	 print *, ' tau =', nint(taub(i)*1.e3), ' tkd05 =', nint(taub_kd05(i)*1.e3), 'Fr=', Fr
!	 print *, ' zogw=', nint(zogw(j)), ' zobl=', nint(zobl(j))          ! nint(mkd05_hills(i)), nint(arhills(i))
!	endif
...
!
!====================================================================================== 	    
!	    zw1 = zw1 * arhills(i)    ! simple scale-awareness nhills=Grid_Area/Hill_area 
!	    apply limiters for OGW tendency
!====================================================================================== 
	    if (abs(zw1) > max_axyz ) then
                zw1 = sign(max_axyz, zw1)
!		if (kdt <=2 ) then
!		    print *, ' Hdudt ', nint(max_axyz*1.e5), nint(zw2*1.e5)
!		    print *, ' Hdudt ', xn(i), yn(i)
!		endif    
            endif 
	        taud(i,k)= zw1
          enddo
...
! ----------------------------------------------
      ...
      if( do_tofd ) then
!      
!  can scale varss(j) by adjusting  filterd oro_turb spectra
!             a1-coeff  by (Lx_flt_cXXX/Lx_c768)^1.9
!                          
!     klow = 6.28/10km of Beljaars_etal_2004  and kflt^n1
!     kflt = 6.28/18km	
...
!        endif
        ...
        do i = 1,npt          
          ...
          sigflt = min(sgh30(j), 0.3*hprime(j)) ! cannot exceed 30% of ls-sso 
	                                        ! GSL-2/limits a) 250 m ; b) var_maxfd =150m
          zsurf = zmeti(j,1)
...
!  GSL-scheme:     varmax_fd, beta_fd ,250.
