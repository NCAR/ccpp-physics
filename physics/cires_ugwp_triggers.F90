...
!
	  ...
	  klev(i) = k	     
	     ...
	     kex =0
	     if (dmax .ge. tlim_fgf) kex = kex+1 
	  do k=klow+1, ktop
	     dtot =abs(trig_fgf(i,k))
	     if (dtot .ge. tlim_fgf ) kex = kex+1 
	     if ( dtot >  dmax) then
	        klev(i) = k	
		dmax =  dtot
	     endif  
