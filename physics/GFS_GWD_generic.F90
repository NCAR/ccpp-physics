...
!> \defgroup gfs_GFS_GWD_generic GFS Orographic Gravity Wave Drag and Mountain Blocking Scheme Module
...
!! | errflg         | ccpp_error_flag                                                               | error flag for error handling in CCPP                                                                    | flag       |    0 | integer   |           | out    | F        |
!!
!> \section gen_GFS_GWD_generic GFS Orographic GWD Scheme General Algorithm
...
!!
!> \section det_GFS_GWD_generic GFS Orographic GWD Scheme Detailed Algorithm
!> @{
...
!-----   GFS_GWD_generic_mb.f version (following lmi) elvmax < hncrit (*j*)
!        MB3a expt to enhance elvmax mtn hgt see sigfac & hncrit
!        GFS_GWD_generic_GWDFIX_v6.f FIXGWD GF6.0 20070608 sigfac=4.
!-----
!----------------------------------------------------------------------C
!    USE
...
!
        ...
        enddo
!       if (lprnt) 
!    &  print *,' in GFS_GWD_generic_lm.f npt,IM,IX,IY,km,me=',npt,IM,IX,IY,km,me
!
!
...
!     print *,' in GFS_GWD_generic_lm.f 1  =',k,ELVMAX(j),pklog,pkp1log,me
! ---        wk for diags but can be saved and reused.  
               wk(i)    = G * ELVMAX(j) / (phil(j,k+1) - phil(j,k))
               iwklm(I) =  MAX(iwklm(I), k+1) 
!     print *,' in GFS_GWD_generic_lm.f 2 npt=',npt,i,j,wk(i),iwklm(i),me
            endif
!
...
!> - Compute Brunt-Vaisala Frequency \f$N\f$.
     ...
     &                                / (VTK(I,Kp1) + VTK(I,K))
          ...
          ENDDO
        ENDDO
!    print *,' in GFS_GWD_generic_lm.f 3 npt=',npt,j,RDZ,me
!
        DO I = 1, npt
          J   = ipt(i)
...
!! the maximum mountain height and processing downward.
        DO Ktrial = KMLL, 1, -1
          ...
          ENDDO
        ENDDO
!     print *,' in GFS_GWD_generic_lm.f 4 npt=',npt,kreflm(npt),me
!
...
! --- these vert ave are for diags, testing and GWD to follow (*j*).
          ENDDO
        ENDDO
!     print *,' in GFS_GWD_generic_lm.f 5  =',i,kreflm(npt),BNV2bar(npt),me
!
...
!! from the U.S. Geological Survey.
        ...
        ENDDO
!
!     print *,' in GFS_GWD_generic_lm.f 6  =',phiang,THETA(ipt(npt)),me
!     print *,' in GFS_GWD_generic_lm.f 7  =',IDXZB(npt),PE(npt)
!
...
!     print *,' in GFS_GWD_generic_lm.f 8 npt =',npt,ZBK(npt),UP(npt),me
!     endif
!
...
!
        DO I = 1, npt
          J = ipt(i)
          ZLEN = 0.
!      print *,' in GFS_GWD_generic_lm.f 9  =',i,j,IDXZB(i),me
          IF (IDXZB(I) > 0) then
            DO K = IDXZB(I), 1, -1
...
!                 print *,' in GFS_GWD_generic_lmi.f 10 npt=',npt,i,j,idxzb(i)
!    &,           DBTMP,R' ang=',ang(i,k),' gamma=',gamma(j),' K=',K
!                 print *,' in GFS_GWD_generic_lmi.f 11   K=',k,ZLEN,cos(ANG(I,K))
!                 print *,' in GFS_GWD_generic_lmi.f 12  DB=',DB(i,k),sin(ANG(I,K))
!               endif
              endif
            ENDDO
...
!         BNV2(I,K) = TEM * (VTK(I,K+1)-VTK(I,K))/(VTK(I,K+1)+VTK(I,K))
     ...
     &                            / (VTK(I,Kp1)+VTK(I,K))
        ...
        ENDDO
      ENDDO
!      print *,' in GFS_GWD_generic_lm.f GWD:14  =',npt,kmm1,bnv2(npt,kmm1)
!
...
!
      ...
      ENDDO
!      print *,' in GFS_GWD_generic_lm.f GWD:15  =',KBPS,KMPS
      KBPSP1 = KBPS + 1
      KBPSM1 = KBPS - 1
      DO K = 1,KBPS
        DO I = 1,npt
...
!
        ...
        ENDDO
      ENDDO
!      print *,' in GFS_GWD_generic_lm.f GWD:15B =',bnv2bar(npt)
!
...
!    & print *,' in GFS_GWD_generic_lmi.f KDT=',KDT,I,K,DB(I,K),
...
!
     ...
     &               + (V1(J,K)+DTAUY*DELTIM)*(V1(J,K)+DTAUY*DELTIM))   
      ...
      ENDDO
!     if (lprnt) then
!       print *,' in GFS_GWD_generic_lm.f after  A=',A(ipr,:)
!       print *,' in GFS_GWD_generic_lm.f after  B=',B(ipr,:)
!       print *,' DB=',DB(ipr,:)
!     endif
...
!     ENDIF
!
!      print *,' in GFS_GWD_generic_lm.f 18  =',A(ipt(1),idxzb(1))
!    &,                          B(ipt(1),idxzb(1)),me
      RETURN
