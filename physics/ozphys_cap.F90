...
!>
!! @brief Auto-generated cap module for the ozphys scheme
!!
!
module ozphys_cap
    ...
    use            :: ozphys, &
                      only: ozphys_init,ozphys_finalize,ozphys_run
    ...
    private
    public :: ozphys_init_cap,ozphys_finalize_cap,ozphys_run_cap

    contains


    function ozphys_init_cap(ptr) bind(c) result(ierr)
    ...
    end function ozphys_init_cap

    function ozphys_finalize_cap(ptr) bind(c) result(ierr)
        ...
        call ozphys_finalize()
        

    end function ozphys_finalize_cap

    function ozphys_run_cap(ptr) bind(c) result(ierr)
        ...
        call ccpp_field_get(cdata, 'number_of_coefficients_in_ozone_forcing_data', oz_coeff, ierr=ierr, kind=ckind, index=574)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve number_of_coefficients_in_ozone_forcing_data from CCPP data structure')
            return
        end if
        if (kind(oz_coeff).ne.ckind) then
        ...
        call ozphys_run(ix=ix,im=im,levs=levs,ko3=ko3,dt=dt,oz=oz,tin=tin,po3=po3,prsl=prsl,prdout=prdout, &
                  oz_coeff=oz_coeff,delp=delp,ldiag3d=ldiag3d,ozp1=ozp1,ozp2=ozp2,ozp3=ozp3, &
                  ozp4=ozp4,con_g=con_g,me=me,errmsg=cdata%errmsg,errflg=cdata%errflg)
        ierr=cdata%errflg

    end function ozphys_run_cap
end module ozphys_cap
