...
!     use cires_ugwp1_sporo,     only: oro_spectral_solver

    use drag_suite, only: drag_suite_run

!    use cires_ugwpv1_triggers, only:  get_spectra_tau_convgw,  get_spectra_tau_okw, get_spectra_tau_nstgw     
...
! old data:	  jindx1_tau, jindx2_tau, ddy_j1tau, ddy_j2tau,                         &	  
...
! SSO parameters and variables
    ...
    real(kind=kind_phys),    intent(in) :: cdmbgwd(4)                      ! in gsl_drag
...
!
! the zero diag and tendency values assigned inside "drag_suite_run" can be skipped :
!
...
!	  pdudt  = 0.0*pdudt ; pdvdt  = 0.0*pdvdt ; pdtdt = 0.
...
!
! updates of MERRA/GEOS tau_ngw for the C96-QBO FV3GFS-127L runs
!================================================================== 
          
       call    slat_geos5_2020(im, tamp_mpa, xlat_d, tau_ngw)
!       if (me == master) then
!          print *, ' ugwpv1 forcing ', maxval(tau_ngw), minval(tau_ngw)
!          print *, ' ugwpv1 forcing tamp_mpa ', tamp_mpa	  
!       endif
