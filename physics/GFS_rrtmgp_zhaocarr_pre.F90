...
! radiation schemes. Only compatable with Model%imp_physics = Model%imp_physics_zhaocarr
! ########################################################################################
module GFS_rrtmgp_zhaocarr_pre
  ...
  use radcons,      only: qmin
  ! Parameters
  real(kind_phys), parameter :: &
       reliq_def = 10.0 ,       & ! fault liq radius to 10 micron
...
!!  
  subroutine GFS_rrtmgp_zhaocarr_pre_run(Model, Tbd, nCol, nLev, lat, lsmask,  &
       p_lev, p_lay, t_lay, relhum, tv_lay, tracer, cld_frac, cld_lwp, cld_reliq, cld_iwp, cld_reice,   &
       cld_swp, cld_resnow, cld_rwp, cld_rerain, errmsg, errflg)
    implicit none
    ...
    do iLay=1,nLev
       do iCol=1,nCol   
          if (cld_condensate(iCol,iLay,1) < epsq) cld_condensate(iCol,iLay,1) = 0.0
       enddo
    enddo
    
    ! Use radii provided from the macrophysics  
    if (Model%effr_in) then
       cld_reliq(1:nCol,1:nLev)  = Tbd%phy_f3d(1:nCol,1:nLev,2)
       cld_reice(1:nCol,1:nLev)  = Tbd%phy_f3d(1:nCol,1:nLev,3)
       cld_rerain(1:nCol,1:nLev) = Tbd%phy_f3d(1:nCol,1:nLev,4)
       cld_resnow(1:nCol,1:nLev) = Tbd%phy_f3d(1:nCol,1:nLev,5)
    else
       cld_reliq(1:nCol,1:nLev)  = reliq_def
       cld_reice(1:nCol,1:nLev)  = reice_def
       cld_rerain(1:nCol,1:nLev) = rerain_def
       cld_resnow(1:nCol,1:nLev) = resnow_def
    endif
    
    ! Use cloud-fraction from SHOC?
    if (Model%uni_cld) then
       cld_frac(1:nCol,1:nLev) = Tbd%phy_f3d(1:nCol,1:nLev,Model%indcld)
    ...
    cld_condensate(1:nCol,1:nLev,1) = cld_condensate(1:nCol,1:nLev,1) + Tbd%phy_f3d(1:nCol,1:nLev,6)
    
    ! Compute cloud liquid/ice condensate path.
    do iLay=1,nLev
       do iCol=1,nCol
          tem1               = max(0.0, cld_condensate(iCol,iLay,1)) * gfac * deltaP(iCol,iLay)
       ...
       do iLay=1,nLev
          do iCol=1,nCol
             ...
             if (cld_iwp(iCol,iLay) > 0.0) then
                tem3 = gord * cld_iwp(iCol,iLay) * p_lay(iCol,iLay) / (deltaP(iCol,iLay)*tv_lay(iCol,iLay))
                if (tem2 < -50.0) then
