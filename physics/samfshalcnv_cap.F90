...
!>
!! @brief Auto-generated cap module for the samfshalcnv scheme
!!
!
module samfshalcnv_cap
    ...
    use            :: samfshalcnv, &
                      only: samfshalcnv_init,samfshalcnv_run,samfshalcnv_finalize
    ...
    private
    public :: samfshalcnv_init_cap,samfshalcnv_run_cap,samfshalcnv_finalize_cap

    contains


    function samfshalcnv_init_cap(ptr) bind(c) result(ierr)
        ...
        call samfshalcnv_init()
        

    end function samfshalcnv_init_cap

    function samfshalcnv_run_cap(ptr) bind(c) result(ierr)
        ...
        call ccpp_field_get(cdata, 'lwe_thickness_of_shallow_convective_precipitation_amount', rn, ierr=ierr, dims=cdims, kind=ckind, index=491)
#ifdef DEBUG
        if (ierr /= 0) then
        ...
        call ccpp_field_get(cdata, 'entrainment_rate_coefficient_shallow_convection', clam, ierr=ierr, kind=ckind, index=255)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve entrainment_rate_coefficient_shallow_convection from CCPP data structure')
            return
        end if
        if (kind(clam).ne.ckind) then
            call ccpp_error('Kind mismatch for variable entrainment_rate_coefficient_shallow_convection')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'rain_conversion_parameter_shallow_convection', c0s, ierr=ierr, kind=ckind, index=615)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve rain_conversion_parameter_shallow_convection from CCPP data structure')
            return
        end if
        if (kind(c0s).ne.ckind) then
            call ccpp_error('Kind mismatch for variable rain_conversion_parameter_shallow_convection')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'detrainment_conversion_parameter_shallow_convection', c1, ierr=ierr, kind=ckind, index=217)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve detrainment_conversion_parameter_shallow_convection from CCPP data structure')
            return
        end if
        if (kind(c1).ne.ckind) then
            call ccpp_error('Kind mismatch for variable detrainment_conversion_parameter_shallow_convection')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'momentum_transport_reduction_factor_pgf_shallow_convection', pgcon, ierr=ierr, kind=ckind, index=556)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve momentum_transport_reduction_factor_pgf_shallow_convection from CCPP data structure')
            return
        end if
        if (kind(pgcon).ne.ckind) then
            call ccpp_error('Kind mismatch for variable momentum_transport_reduction_factor_pgf_shallow_convection')
            ierr = 1
            return
        end if
#endif
        ...
        call ccpp_field_get(cdata, 'aerosol_aware_parameter_shallow_convection', asolfac, ierr=ierr, kind=ckind, index=36)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve aerosol_aware_parameter_shallow_convection from CCPP data structure')
            return
        end if
        if (kind(asolfac).ne.ckind) then
            call ccpp_error('Kind mismatch for variable aerosol_aware_parameter_shallow_convection')
            ierr = 1
            return
        end if
#endif
        ...
        call samfshalcnv_run(im=im,ix=ix,km=km,cliq=cliq,cp=cp,cvap=cvap,eps=eps,epsm1=epsm1,fv=fv,grav=grav, &
                  hvap=hvap,rd=rd,rv=rv,t0c=t0c,delt=delt,ntk=ntk,ntr=ntr,delp=delp,prslp=prslp, &
                  psp=psp,phil=phil,qtr=qtr,q1=q1,t1=t1,u1=u1,v1=v1,rn=rn,kbot=kbot,ktop=ktop, &
                  kcnv=kcnv,islimsk=islimsk,garea=garea,dot=dot,ncloud=ncloud,hpbl=hpbl,ud_mf=ud_mf, &
                  dt_mf=dt_mf,cnvw=cnvw,cnvc=cnvc,clam=clam,c0s=c0s,c1=c1,pgcon=pgcon,asolfac=asolfac, &
                  errmsg=cdata%errmsg,errflg=cdata%errflg)
        ierr=cdata%errflg

    end function samfshalcnv_run_cap

    function samfshalcnv_finalize_cap(ptr) bind(c) result(ierr)
        ...
        call samfshalcnv_finalize()
        

    end function samfshalcnv_finalize_cap
end module samfshalcnv_cap
