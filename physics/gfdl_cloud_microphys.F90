...
module gfdl_cloud_microphys_pre
   ...
   use physcons,              only: con_fvirt, con_g, con_rd
...
!! |-------------|---------------|---------------------------------------------|---------|------|-----------|--------|--------|----------|
!! | do_shock    | do_shock_flag | flag to indicated use of SHOCK              | flag    |    0 | logical   |        | in     | F        |
...
!!
      ...
      logical         , intent( in) :: do_shoc         !< flag for SHOC
...
!!
      subroutine gfdl_cloud_microphys_pre_run(                                      &         
            ...
            pt, w, uin, vin, delp, dz, errmsg, errflg)
...
!! flip vertical (k) coordinate, and add third index.  
            qv1  (i,1,k) = gq0(i,levs-k+1)
            ql1  (i,1,k) = gq0_ntcw(i,levs-k+1)
            qr1  (i,1,k) = gq0_ntrw(i,levs-k+1)
            qi1  (i,1,k) = gq0_ntiw(i,levs-k+1)
            qs1  (i,1,k) = gq0_ntsw(i,levs-k+1)
            qg1  (i,1,k) = gq0_ntgl(i,levs-k+1)
            qa1  (i,1,k) = gq0_ntclamt(i,levs-k+1)
            pt   (i,1,k) = gt0(i,levs-k+1)
            w    (i,1,k) = -vvl(i,levs-k+1) * ( one+con_fvirt * qv1(i,1,k) )   &
     &               *  gt0(i,levs-k+1) / prsl(i,levs-k+1) * (con_rd*onebg)
...
!!
!!
      subroutine gfdl_cloud_microphys_pre_finalize (errmsg, errflg)
...
!!
!!
      subroutine gfdl_cloud_microphys_post_init (errmsg, errflg)
...
!
      ...
      do i = 1, im
        ...
        ice(i)     = ice0    (i,1) * tem
        snow(i)    = snow0   (i,1) * tem
        graupel(i) = graupel0(i,1) * tem
...
!
      do k = 1, levs
        ...
        do i=1,im
            ...
            gq0_ntsw(i,k)    = qs1(i,1,kk) + qs_dt(i,1,kk) * dtp
            gq0_ntgl(i,k)    = qg1(i,1,kk) + qg_dt(i,1,kk) * dtp
            gq0_ntclamt(i,k) = qa1(i,1,kk) + qa_dt(i,1,kk) * dtp
            gt0(i,k)         = gt0(i,k)    + pt_dt(i,1,kk) * dtp
            gu0(i,k)         = gu0(i,k)    + udt  (i,1,kk) * dtp
            gv0(i,k)         = gv0(i,k)    + vdt  (i,1,kk) * dtp
        enddo
      enddo
...
!! |-------------|------------------------------------|-----------------------------------------|-----------|-------|--------|----------|
!!
      subroutine gfdl_cloud_microphys_post_finalize (errmsg, errflg)
...
!! |----------------|--------------------------------------------------|----------------------------------------------------|--------|------|-----------|-------|--------|----------|
!!
subroutine gfdl_cloud_microphys_init (me, master, nlunit, input_nml_file, logunit, fn_nml, errmsg, errflg)
