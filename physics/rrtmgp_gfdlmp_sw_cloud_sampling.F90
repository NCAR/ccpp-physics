...
!!
  subroutine rrtmgp_gfdlmp_sw_cloud_sampling_init(sw_gas_props, ipsdsw0, errmsg, errflg)
    ! Inputs
...
!!
  subroutine rrtmgp_gfdlmp_sw_cloud_sampling_run(doSWrad, nCol, nDay, nLev, ipsdsw0, idxday,       &
    ...
    if (iovrsw .ne. 1 .and. iovrsw .ne. 3 .and. iovrsw .ne. 4 .an. iovrsw .ne. 5) then
    ...
    if (nDay .gt. 0) then
       ...
       case(5) ! Exponential-random overlap
          call check_error_msg('rrtmgp_gfdlmp_sw_cloud_sampling_run', &
               sampled_mask_exp_ran(rng3D,cld_frac(idxday(1:nDay),:),cldfracMCICA))
       end select
