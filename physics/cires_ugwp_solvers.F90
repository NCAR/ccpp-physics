...
!            
!     
     subroutine ugwp_oro(im, levs, ntrac, dtp, kdt,me, lprnt, fcor, c2f2,     &
       u, v, tkin, qtrac, pint, delp, pmid, pexner, gzint, gzmid, orostat,    &   
...
!----------------------------------------------------------------------      
      ...
      real(kind_phys), dimension(im, levs)        ::   u, v, tkin
      real(kind_phys), dimension(im, levs, ntrac) ::   qtrac        
...
!      
      ...
      integer   ::  npt,    ktrial, klevm1, nmtvr
!
      integer   ::  krefj, kdswj, kotr
      integer   ::  i, j, k 
      integer   ::  ipt(im)
...
!  
        tautot = 0.;  tauogw=0; taumtb =0. ; taulee=0.;  taurf=0.0 
         axz =0.
	 ayz =0.
	 edis =0.
         kdis =0.
	 krf2d =0.
         ...
         dusfc    =0.    ; dvsfc    =0.
         dusfc_mb =0.    ; dvsfc_mb =0.
	 dusfc_ogw =0.   ; dvsfc_ogw =0.
	 dusfc_lwb =0.	 ; dvsfc_lwb =0.
         dusfc_tofd =0.0 ; dvsfc_tofd =0.0	          
         zmtb =- 99.
	 zlwb = -99.
	 zogw = -99.	 	 
         tauf_ogw(:) = 0.
...
!
        ...
        if (npt .eq. 0) return       ! no ororgraphy  ====> gwd/mb calculation done
...
!
           ...
           up(1:levs)     =u(j,1:levs)
           vp(1:levs)     =v(j,1:levs)
           tp(1:levs)     =tkin(j,1:levs)
           qp(1:levs)     =qtrac(j,1:levs,1)       ! H2O-index -1 in tracer-array
           dp(1:levs)     =delp(j,1:levs)
	   
           zpm(1:levs)    = gzmid(j,1:levs)  *rgrav
           zpi(1:levs+1)  = gzint(j,1:levs+1)*rgrav
	   
           pmid1(1:levs)   =pmid(j,1:levs)
           pint1(1:levs+1) =pint(j,1:levs+1)
           pex(1:levs)     =pexner(j,1:levs)
!
! elvp- k-index: iwklm k_elvp = index for elvmax + 4*hprime, "elevation index" 
! GFS-2017
          do k=1, levs-1
            if (elvp .le. zpi(k+1) .and. ( elvp .gt. zpi(k)  ) ) then
...
!
        drmtb =0. ; drlee =0. ; drtau =0. ; drlow = 0.
!
...
!
      if ( (krefj .gt. 1) .and. ( abs(tauogw(j)) .gt. 0.) ) then
!
...
!	
	  ...
	  eng1       = 0.
!
          if (k .lt. k_mtb(i) .and. drmtb(k) .ne. 0 ) then
	    loss     = 1./ (1.+drmtb(k)*dtp)
...
!	    
            eng1  = (u(j,k)+dtaux*dtp)*(u(j,k)+dtaux*dtp) &
                  + (v(j,k)+dtauy*dtp)*(v(j,k)+dtauy*dtp) +eng1
     
            dusfc(j)   = dusfc(j)  + dtaux * dp(k)
            dvsfc(j)   = dvsfc(j)  + dtauy * dp(k)
	   	  
            edis(j,k)  = max(eng0-eng1,0.)*r_cpdt      !+ epstofd1(k)
...
!      
!	   
      subroutine gw_solver_linsatdis(im, levs, ntrac, dtp, kdt, me,   &
...
!==========================
        real, dimension(im, levs)   ::  u, v, t, delp, prsl, prslk, phil
	real, dimension(im, levs, ntrac)   :: q 	
...
!==========================	
	...
	real, dimension(levs)   ::  q1 	
        real, dimension(levs+1) ::  pint , zint	
...
!
           u1(1:levs)     =u(j,1:levs)
           v1(1:levs)     =v(j,1:levs)
           t1(1:levs)     =t(j,1:levs)
           q1(1:levs)     =q(j,1:levs,1)       ! H2O-index -1 in tracer-array
           dp(1:levs)     =delp(j,1:levs)
	   
           zmid(1:levs)    = phil(j,1:levs)  *rgrav
           zint(1:levs+1)  = phii(j,1:levs+1)*rgrav	   
           pmid(1:levs)    =  prsl(j,1:levs)
           pint(1:levs+1)  =  prsi(j,1:levs+1)
!          pex1(1:levs)    =  prslk(j,1:levs)
...
!
    subroutine gw_solver_wmsdis(im, levs, ntrac, dtp, kdt, me,        &
        taub, klev, if_src, nf_src, nw,  ch, naz,  spf,  xaz, yaz,    &                                 
        fcor, c2f2, u, v, t,  q, prsi, delp, prsl, prslk, phii, phil, &
        ax, ay, eps, ked, tauz)
...
!
      ...
      real, dimension(im, levs) :: u, v
      real, dimension(im, levs) :: ax, ay, eps
      real :: ud, vd  
      integer    :: i, k  
