...
!..OPTIONAL variables that control application of aerosol-aware scheme
      ...
      if (.NOT.PRESENT(is_start)) then
         write(*,*) "thompson_init: optional argument is_start required if is_aerosol_aware is true"
         return
      end if
...
! DH*
! Allow for external aerosol surface emission rates, fallback to original
! approach by Greg if none are provided.
! *DH
...
!.. for a grid with 20km spacing and scale accordingly for other spacings.
      ...
      max_test = MAXVAL ( nwfa2d(its:ite,jts:jte) )

      if (is_start .and. max_test.lt.eps) then
         write(*,*) ' Apparently CCN emission rates are not specified.'
         write(*,*) '   checked column at point (i,j) = ', its,jts
         write(*,*) '   calculate emission rates from initial CCN distribution'
...
!+---+-----------------------------------------------------------------+
      ...
      if (mpirank==mpiroot) WRITE (*,*) ' using: mu_c=',mu_c,' mu_i=',mu_i, &
                                        ' mu_r=',mu_r,' mu_g=',mu_g
...
!     endif
      ...
      i_loop:  do i = i_start, i_end
         ...
         IF ( PRESENT(snowncv) .AND. PRESENT(snownc) ) THEN
            ! Add ice to snow if separate ice not present (as in WRF)
