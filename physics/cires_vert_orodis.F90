...
!
      ...
      real ::  bn2(nz), uds(nz), umf(nz)
      real ::  cosang(nz), sinang(nz)
      
      integer :: k,  klow, ktop, kpbl
      real    :: uhm, vhm, bn2hm, rhohm
      real    :: mtb_fix, umag, bnmag, frd_src
      real    :: zblk, who_iz_normal, rlm97
      real    :: phiang, ang, pe, ek      
      real    :: cang, sang, ss2, cs2, zlen,  dbtmp   
      real    :: hamp    
      real    :: bgamm, cgamm      
...
!
	    ...
	    if ( zblk .lt.zpm_zero(k) .and. zblk .ge.zpm_zero(k-1)) then 
                 idxzb = k 
            exit
            endif
      	  enddo  
!   
	  ...
	  if (idxzb .gt. 1) then      ! let start with idxzb = 2....and up with LM1997
!
...
!
      ...
      call um_flow(nz, klow, ktop, up, vp, tp, qp, dp, zpm, zpi, pmid, pint,  &
           bn2, uhm, vhm, bn2hm, rhohm)
       if (bn2hm .le. 0.0) RETURN                        ! unstable PBL   
!--------------------------------------------- 	   
!
...
!	     
      ...
      if (bn2hm .le. 0.0) RETURN        !  "unstable/neutral" hill need different treatment
      bnv = sqrt(bn2hm)
      ...
      if (umag2 .le. 0.0)  RETURN      ! Coriolis cut-off at high-mid latitudes for low kx
      ...
      IF (mkz2.lt. mkz2min .or. krefj <= 2 )  THEN
...
! 
       if (do_klwb_phase) then
           ...
           else if ( k.ge. klwb .and. k .lt. kotr) then
               tau_src(k)  = tauogw
                 drlee(k)  = 0.0
           endif
         enddo  
...
! above izb  tau
           ...
           mkz2 = bn2i(k)/umd2 - kx2
            if ( mkz2 .gt. mkz2min ) then
             mkz  = sqrt(mkz2)
             frlocal = max(hdsw*bvi(k)/umd(k), frlwb) 
             phmkz(k) = phmkz(k-1) + mkz*(zpm(k)-zpm(k-1))
             if( ( phmkz(k) .ge. zbr_pi ) .and. (frlocal > frlin)) klwb = k              
            else 
               kotr = k
               exit loop_lwb_gfs18
            endif
...
!
           uf2p  = uf2 + 2.0*c2f2
           betadis = uf2/uf2p
           betaM =1./(1.+betadis)       ! if c2f2 = 0. betaM = betaT  =0.5 ekw = epw
           betaT =1.- BetaM
