...
!>
!! @brief Auto-generated cap module for the satmedmfvdif scheme
!!
!
module satmedmfvdif_cap
    ...
    use            :: satmedmfvdif, &
                      only: satmedmfvdif_init,satmedmfvdif_finalize,satmedmfvdif_run
    ...
    private
    public :: satmedmfvdif_init_cap,satmedmfvdif_finalize_cap,satmedmfvdif_run_cap

    contains


    function satmedmfvdif_init_cap(ptr) bind(c) result(ierr)
        ...
        call satmedmfvdif_init()
        

    end function satmedmfvdif_init_cap

    function satmedmfvdif_finalize_cap(ptr) bind(c) result(ierr)
        ...
        call satmedmfvdif_finalize()
        

    end function satmedmfvdif_finalize_cap

    function satmedmfvdif_run_cap(ptr) bind(c) result(ierr)
        ...
        call ccpp_field_get(cdata, 'index_for_turbulent_kinetic_energy_vertical_diffusion_tracer', ntke, ierr=ierr, kind=ckind, index=393)
#ifdef DEBUG
        if (ierr /= 0) then
        ...
        call ccpp_field_get(cdata, 'latent_heat_of_fusion_of_water_at_0C', hfus, ierr=ierr, kind=ckind, index=458)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve latent_heat_of_fusion_of_water_at_0C from CCPP data structure')
            return
        end if
        if (kind(hfus).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'tendency_of_air_temperature_due_to_model_physics', tdt, ierr=ierr, dims=cdims, kind=ckind, index=758)
#ifdef DEBUG
        if (ierr /= 0) then
            call ccpp_error('Unable to retrieve tendency_of_air_temperature_due_to_model_physics from CCPP data structure')
            return
        end if
        if (kind(tdt).ne.ckind) then
        ...
        call ccpp_field_get(cdata, 'tendency_of_tracers_due_to_model_physics', rtg, ierr=ierr, dims=cdims, kind=ckind, index=776)
#ifdef DEBUG
        if (ierr /= 0) then
        ...
        call ccpp_field_get(cdata, 'tracer_concentration', q1, ierr=ierr, dims=cdims, kind=ckind, index=801)
#ifdef DEBUG
        if (ierr /= 0) then
        ...
        call satmedmfvdif_run(ix=ix,im=im,km=km,ntrac=ntrac,ntcw=ntcw,ntiw=ntiw,ntke=ntke,grav=grav,rd=rd, &
                  cp=cp,rv=rv,hvap=hvap,hfus=hfus,fv=fv,eps=eps,epsm1=epsm1,dv=dv,du=du,tdt=tdt, &
                  rtg=rtg,u1=u1,v1=v1,t1=t1,q1=q1,swh=swh,hlw=hlw,xmu=xmu,garea=garea,psk=psk, &
                  rbsoil=rbsoil,zorl=zorl,u10m=u10m,v10m=v10m,fm=fm,fh=fh,tsea=tsea,heat=heat, &
                  evap=evap,stress=stress,spd1=spd1,kpbl=kpbl,prsi=prsi,del=del,prsl=prsl, &
                  prslk=prslk,phii=phii,phil=phil,delt=delt,dspheat=dspheat,dusfc=dusfc,dvsfc=dvsfc, &
                  dtsfc=dtsfc,dqsfc=dqsfc,hpbl=hpbl,kinver=kinver,xkzm_m=xkzm_m,xkzm_h=xkzm_h, &
                  xkzm_s=xkzm_s,errmsg=cdata%errmsg,errflg=cdata%errflg)
        ierr=cdata%errflg

    end function satmedmfvdif_run_cap
end module satmedmfvdif_cap
