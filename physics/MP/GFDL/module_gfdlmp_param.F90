...
module module_gfdlmp_param
  ...
  type ty_gfdlmp_config
     ...
     real :: tice_mlt    = 273.16  !< can set ice melting temperature to 268 based on observation (Kay et al. 2016) (K) (v3 only)
     ...
     real :: tau_v2g     = 21600.  !< graupel deposition -- make it a slow process (v1 only)
     real :: tau_gmlt    = 600.0   !< graupel melting time scale (s) (v3 only)
     real :: tau_wbf     = 300.0   !< graupel melting time scale (s) (v3 only)
     ...
     real :: qc_crt      = 5.0e-8  !< mini condensate mixing ratio to allow partial cloudiness (v1 only)
     ...
     real :: qi_gen      = 1.82e-6 !< max cloud ice generation during remapping step (V1 ONLY. Computed internally in V3) (v1/v3)
     ...
     real :: qr0_crt     = 1.0e-4  !< rain to snow or graupel/hail threshold (v1 only)
     ...
     real :: c_psacw     = 1.0     !< cloud water to snow accretion efficiency (v3 only)
     real :: c_psaci     = 0.02    !< accretion: cloud ice to snow (was 0.1 in zetac) (v3= = 0.05)
     real :: c_pracw     = 0.8     !< cloud water to rain accretion efficiency (v3 only)
     real :: c_praci     = 1.0     !< cloud ice to rain accretion efficiency (v3 only)
     real :: c_pracs     = 1.0     !< snow to rain accretion efficiency (v3 only)
     real :: c_piacr     = 5.0     !< accretion: rain to ice: (v1 only)
     real :: c_cracw     = 0.9     !< rain accretion efficiency (v1 only)
     real :: c_pgacs     = 2.0e-3  !< snow to graupel "accretion" eff. (was 0.1 in zetac) (v3 = 0.01)
     real :: c_pgacw     = 1.0     !< cloud water to graupel accretion efficiency  (v3 only)
     real :: c_psacr     = 1.0     !< rain to snow accretion efficiency  (v3 only) 
     real :: c_pgaci     = 0.05    !< cloud ice to graupel accretion efficiency (was 0.1 in ZETAC) (v3 only)
     real :: c_pgacr     = 1.0     !< rain to graupel accretion efficiency  (v3 only)
     real :: is_fac      = 0.2     !< cloud ice sublimation temperature factor (v3 only)
     real :: ss_fac      = 0.2     !< snow sublimation temperature factor (v3 only)
     real :: gs_fac      = 0.2     !< graupel sublimation temperature factor (v3 only)
     real :: rh_fac_evap = 10.0    !< cloud water evaporation relative humidity factor (v3 only)
     real :: rh_fac_cond = 10.0    !< cloud water condensation relative humidity factor (v3 only)
     real :: alin        = 842.0   !< "a" in lin1983 (v1 only)
     real :: clin        = 4.8     !< "c" in lin 1983, 4.8 -- > 6. (to ehance ql -- > qs) (v1 only)
     ...
     !real :: n0w_exp     = 91      ! intercept parameter (exponent) of cloud water (Lin et al. 1983) (1/m^4) (Martin et al. 1994) (v3)
     real :: n0i_exp     = 18      ! intercept parameter (exponent) of cloud ice (Lin et al. 1983) (1/m^4) (McFarquhar et al. 2015) (v3)
     !real :: n0i_exp     = 17      ! intercept parameter (exponent) of cloud ice (Lin et al. 1983) (1/m^4) (McFarquhar et al. 2015) (v3)
     ...
     !real :: muw         = 16.0    ! shape parameter of cloud water in Gamma distribution (Martin et al. 1994) (v3)
     real :: mui         = 3.35    ! shape parameter of cloud ice in Gamma distribution (McFarquhar et al. 2015) (v3)
     !real :: mui         = 3.54    ! shape parameter of cloud ice in Gamma distribution (McFarquhar et al. 2015) (v3)
     ...
     !real :: n0w_sig     = 1.4     !< intercept parameter (significand) of cloud water (Lin et al. 1983) (1/m^4) (Martin et al. 1994) (v3)
     real :: n0i_sig     = 1.3     !< intercept parameter (significand) of cloud ice (Lin et al. 1983) (1/m^4) (McFarquhar et al. 2015) (v3)
     !real :: n0i_sig     = 9.4    !< intercept parameter (significand) of cloud ice (Lin et al. 1983) (1/m^4) (McFarquhar et al. 2015) (v3)
     ...
     integer :: icloud_f  = 0  !< GFDL cloud scheme (v1/v3) (v3)
                               !< 0: subgrid variability based scheme
                               !< 1: same as 0, but for old fvgfs implementation
                               !< 2: binary cloud scheme
                               !< 3: extension of 0
     integer :: irain_f   = 0  !< cloud water to rain auto conversion scheme (v1/v3) (v3)
                               !< 0: subgrid variability based scheme
     ...
     integer :: rewflag   = 1  !< cloud water effective radius scheme (v3)
                               ...
                               !< 3: Kiehl et al. (1994)
                               !< 4: effective radius
     integer :: reiflag   = 5  !< cloud ice effective radius scheme (v3)
