...
! </table>
    ...
    public fv_sat_adj_v3_init, fv_sat_adj_v3_run, fv_sat_adj_v3_finalize
...
!>\brief The subroutine 'fv_sat_adj_v3_init' initializes lookup tables for the saturation mixing ratio.
!! \section arg_table_fv_sat_adj_v3_init Argument Table
!! \htmlinclude fv_sat_adj_v3_init.html
!!
subroutine fv_sat_adj_v3_init(do_sat_adj, kmp, nwat, ngas, rilist, cpilist, &
    ...
    if (.not.do_sat_adj) then
      write(errmsg,'(a)') 'Logic error: fv_sat_adj_v3_init is called but do_sat_adj is set to false'
      errflg = 1
      return
    end if

    if (.not.nwat==6) then
      write(errmsg,'(a)') 'Logic error: fv_sat_adj_v3 requires six water species (nwat=6)'
      errflg = 1
      return
    end if
...
end subroutine fv_sat_adj_v3_init

!\ingroup fast_sat_adj
!>\brief The subroutine 'fv_sat_adj_v3_finalize' deallocates lookup tables for the saturation mixing ratio.
!! \section arg_table_fv_sat_adj_v3_finalize Argument Table
!! \htmlinclude fv_sat_adj_v3_finalize.html
!!
subroutine fv_sat_adj_v3_finalize (errmsg, errflg)
...
end subroutine fv_sat_adj_v3_finalize

!>\defgroup fast_sat_adj GFDL In-Core Fast Saturation Adjustment Module
!> @{
!! The subroutine 'fv_sat_adj_v3' implements the fast processes in the GFDL
!! Cloud MP. It is part of the GFDL Cloud MP.
!>\author Shian-Jiann Lin, Linjiong Zhou
!!
!>\brief The subroutine 'fv_sat_adj_v3' performs the fast processes in the GFDL microphysics.
...
!!
!! \section arg_table_fv_sat_adj_v3_run Argument Table
!! \htmlinclude fv_sat_adj_v3_run.html
!!
subroutine fv_sat_adj_v3_run(mdt, zvir, is, ie, isd, ied, kmp, km, kmdelz, js, je, jsd, jed, &
...
!$OMP do
    do k=kmp,km
       ...
       call fv_sat_adj_v3_work(abs(mdt), zvir, is, ie, js, je, ng, hydrostatic, fast_mp_consv, &
                            te0(isd,jsd,k),                                                 &
#ifdef MULTI_GASES
...
end subroutine fv_sat_adj_v3_run
...
!> @{
subroutine fv_sat_adj_v3_work(mdt, zvir, is, ie, js, je, ng, hydrostatic, consv_te, te0, &
#ifdef MULTI_GASES
        qvi, &
#else 
        qv, &
    ...
    sat_adj0 = 0.9                    ! !< adjustment factor (0: no, 1: full) during fast_sat_adj
...
end subroutine fv_sat_adj_v3_work
!> @}
