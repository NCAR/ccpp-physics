...
!!
!!  WeiGuo Wang updated the scheme for HAFS in July, 2019.
...
!!  @{
      subroutine hedmf_hafs_run (ix,im,km,ntrac,ntcw,dv,du,tau,rtg,          &
...
! RGF determine wspd at roughly 500 m above surface, or as close as
! possible,
! reuse SPDK2
!  zi(i,k) is AGL, right?  May not matter if applied only to water grid
!  points
...
!           if(i.eq.25) print *,' IK ',i,k,' ZI ',zi(i,k), ' WSPM1
!           ',wspm(i,1),'
...
!    1             (hpbl(i)-zl(i,1))), zfmin)
            zfac = max((1.-zi(i,k+1)/hpbl(i)), zfmin)
           ! tem = zi(i,k+1) * (zfac**pfac) * moninq_fac ! lmh suggested
           ! by kg
...
! (3) only do  this IF KPBL(I) >= kLOC.  Otherwise, we are finished,
! with DKU as
! if alpha = +1
...
!  if DKU at 500-m level > WSPM(i,1), then alpha = WSPM(i,1)/xDKU for
!  entire
!  column
            if(xDKU.ge.WSPM(i,1)) then ! ONLY if DKU at 500-m exceeds
cap, otherwise already done

            WSPM(i,3) = WSPM(i,1)/xDKU  ! ratio of cap to Km at k-level, store in WSPM(i,3)
            !WSPM(i,4) = amin1(WSPM(I,3),1.0) ! this is new column
            !alpha. cap at 1. ! should never be needed
            WSPM(i,4) = min(WSPM(I,3),1.0) ! this is new column alpha.
cap at 1. ! should never be needed
...
!!!! CHANGES FOR HEIGHT-DEPENDENT K ADJUSTMENT, WANG W
     ...
     &                  *( 1.0 - ashape )  )
                 tem = zi(i,k+1) * (zfac) * ashape
!                 if(k ==5) write(0,*)'min alf, height-depend
!                 alf',WSPM(i,4),ashape
                endif  ! endif useshape=1
