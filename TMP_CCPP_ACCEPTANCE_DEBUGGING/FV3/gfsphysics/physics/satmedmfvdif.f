...
!************************************************************************
      ...
      ! DH*
      write(0,'(a,2i20)')    "DH IN  kpbl: ", sum(kpbl ),               &
     &                sum(kpbl)
      write(0,'(a,e45.35,i20)') "DH IN  dv:   ", sum(dv   ),            &
     &                chksum(size(dv   ),reshape(dv   ,(/size(dv   )/)))
      write(0,'(a,e45.35,i20)') "DH IN  du:   ", sum(du   ),            &
     &                chksum(size(du   ),reshape(du   ,(/size(du   )/)))
      write(0,'(a,e45.35,i20)') "DH IN  tdt:  ", sum(tdt  ),            &
     &                chksum(size(tdt  ),reshape(tdt  ,(/size(tdt  )/)))
      write(0,'(a,e45.35,i20)') "DH IN  rtg:  ", sum(rtg  ),            &
     &                chksum(size(rtg  ),reshape(rtg  ,(/size(rtg  )/)))
      write(0,'(a,e45.35,i20)') "DH IN  dusfc:", sum(dusfc),            &
     &                chksum(size(dusfc),reshape(dusfc,(/size(dusfc)/)))
      write(0,'(a,e45.35,i20)') "DH IN  dvsfc:", sum(dvsfc),            &
     &                chksum(size(dvsfc),reshape(dvsfc,(/size(dvsfc)/)))
      write(0,'(a,e45.35,i20)') "DH IN  dtsfc:", sum(dtsfc),            &
     &                chksum(size(dtsfc),reshape(dtsfc,(/size(dtsfc)/)))
      write(0,'(a,e45.35,i20)') "DH IN  dqsfc:", sum(dqsfc),            &
     &                chksum(size(dqsfc),reshape(dqsfc,(/size(dqsfc)/)))
      write(0,'(a,e45.35,i20)') "DH IN  hpbl: ", sum(hpbl ),            &
     &                chksum(size(hpbl ),reshape(hpbl ,(/size(hpbl )/)))
      ! *DH
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      ! DH*
      write(0,'(a,2i20)')    "DH OUT kpbl: ", sum(kpbl ),               &
     &                sum(kpbl)
      write(0,'(a,e45.35,i20)') "DH OUT dv:   ", sum(dv   ),            &
     &                chksum(size(dv   ),reshape(dv   ,(/size(dv   )/)))
      write(0,'(a,e45.35,i20)') "DH OUT du:   ", sum(du   ),            &
     &                chksum(size(du   ),reshape(du   ,(/size(du   )/)))
      write(0,'(a,e45.35,i20)') "DH OUT tdt:  ", sum(tdt  ),            &
     &                chksum(size(tdt  ),reshape(tdt  ,(/size(tdt  )/)))
      write(0,'(a,e45.35,i20)') "DH OUT rtg:  ", sum(rtg  ),            &
     &                chksum(size(rtg  ),reshape(rtg  ,(/size(rtg  )/)))
      write(0,'(a,e45.35,i20)') "DH OUT dusfc:", sum(dusfc),            &
     &                chksum(size(dusfc),reshape(dusfc,(/size(dusfc)/)))
      write(0,'(a,e45.35,i20)') "DH OUT dvsfc:", sum(dvsfc),            &
     &                chksum(size(dvsfc),reshape(dvsfc,(/size(dvsfc)/)))
      write(0,'(a,e45.35,i20)') "DH OUT dtsfc:", sum(dtsfc),            &
     &                chksum(size(dtsfc),reshape(dtsfc,(/size(dtsfc)/)))
      write(0,'(a,e45.35,i20)') "DH OUT dqsfc:", sum(dqsfc),            &
     &                chksum(size(dqsfc),reshape(dqsfc,(/size(dqsfc)/)))
      write(0,'(a,e45.35,i20)') "DH OUT hpbl: ", sum(hpbl ),            &
     &                chksum(size(hpbl ),reshape(hpbl ,(/size(hpbl )/)))
      ! *DH
      ...
      function chksum(N, arr) result(hash)
      use machine, only: kind_phys
      implicit none
      integer, intent(in) :: N
      real(kind_phys), dimension(1:N), intent(in) :: arr
      integer*8, dimension(1:N) :: int_arr
      ...
      i=1
      hash = 0
      int_arr = TRANSFER(arr, a, N)

      do i= 1, N
          a = MOD(a + int_arr(i), mod_adler)
          b = MOD(b+a, mod_adler)
      end do
